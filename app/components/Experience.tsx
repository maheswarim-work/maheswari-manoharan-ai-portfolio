              ...
              <div key={index} className="mb-8 p-6 rounded-lg transition-all hover:shadow-lg" style={{
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-2 gap-2">
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xl font-semibold" style={{color: '#2563EB'}}>{job.position}</h4>
                    <div className="flex items-center mt-1" style={{color: '#374151'}}>
                      <Building2 size={16} className="mr-2 flex-shrink-0" style={{color: '#14B8A6'}} />
                ...
                <ul className="mt-4 space-y-2">
                  {job.highlights.map((highlight, hIndex) => (
                    <li key={hIndex} className="text-sm flex" style={{color: '#1E293B'}}>
                      <span className="mr-2 flex-shrink-0" style={{color: '#2563EB', fontWeight: '700'}}>•</span>
                      <span>{highlight}</span>
                    </li>
                  ))}
                </ul>
          ...
          <div>
            ...
            {education.map((edu, index) => (
              <div key={index} className="mb-6 p-6 rounded-lg transition-all hover:shadow-lg" style={{
                ...
                <h4 className="text-xl font-semibold" style={{color: '#2563EB'}}>{edu.degree}</h4>
